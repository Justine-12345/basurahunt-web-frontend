...
import { findEmail, register, loadUser, clearErrors } from '../../actions/userActions'
...
const Register = () => {
    ...
    const [avatarPreview, setAvatarPreview] = useState('https://res.cloudinary.com/basurahunt/image/upload/v1663916552/BasuraHunt/Static/user-default_mdgd40.png?fbclid=IwAR2Ov8lrejcCDxJYRNpDgjF6cWa8Fs-675QUMKzXOzRuvHup3iU1MhhlwaM')

    // const [valid_id, setValid_id] = useState('')
    // const [valid_idPreview, setValid_idPreview] = useState('')

    const [availableEmail, setAvailableEmail] = useState(null)
    const [isValidPassword, setIsValidPassword] = useState(false)

    const [imCollector, setImCollector] = useState(false)

    const backSection = (from, to) => {
        document.getElementById(to).style.display = "block";
        document.getElementById(from).style.display = "none";
        document.getElementById(from + "-arrow").style.filter = "grayscale(100%)";
        document.getElementById(to + "-arrow").style.filter = "grayscale(0%)";
    }
    ...
    const nextSection = (from, to) => {
        console.log(jobDesc)
        if (from === "personal") {
            const new_phone_number = phone_number && phone_number.replace('63', '');
            ...
            if (first_name === "") {
                ...
                alert.error("You Must be 18 years old or above to enter this site")
            }else {
                document.getElementById(to).style.display = "block";
                document.getElementById(from).style.display = "none";
                document.getElementById(from + "-arrow").style.filter = "grayscale(100%)";
        ...
        if (from === "address") {
            ...
            if (house_number === '') {
                alert.error("Please Enter Your House Number")
            }
            else if (street === "") {
                alert.error("Please Enter Your Street")
            }
            else if (barangay === "") {
                alert.error("Please Enter Your Barangay")
            }
            else {
                document.getElementById(to).style.display = "block";
                document.getElementById(from).style.display = "none";
        ...
        if (from === "account") {
            ...
            if (email === '') {
                ...
                alert.error("Please Enter Your Alias")
            }
            else if (password === "") {
                alert.error("Please Enter Your Password")
            }
            else if (isValidPassword === false) {
                alert.error("Please Check Your Password Again")
            }
            else if (confirm_password !== password) {
                alert.error("Incorrect Password Please Confirm Again")
            }
            else {
    ...
    const onChange = async (e) => {
        if (e.target.name === 'avatar') {
            ...
            const lettersForSuffix = /(^[a-zA-Z -.]{0,5})$/

            if (inputName === "first_name" || inputName === "middle_name" || inputName === "last_name") {
                if (inputValue.match(letters) !== null) {
                    setUser({ ...user, [e.target.name]: capitalizeFirstLetter(e.target.value) })
                }
            }
            else if (inputName === "suffix") {
                if (inputValue.match(lettersForSuffix) !== null) {
                    setUser({ ...user, [e.target.name]: capitalizeFirstLetter(e.target.value) })
                }
            } else if (inputName === "house_number") {
                setUser({ ...user, [e.target.name]: capitalizeLetter(e.target.value) })
            }
            else {
    ...
    useEffect(() => {
        ...
        console.log("Json user", JSON.parse(localStorage.getItem("user")))
        if (isAuthenticated || localStorage.getItem("isAuthenticated")) {
            if ((authUser && authUser.otp_status === "Verified") || JSON.parse(localStorage.getItem("user")).otp_status === "Verified") {
                dispatch(loadUser())
                if ((authUser && authUser.role === "administrator") || JSON.parse(localStorage.getItem("user")).role === "administrator") {
                    navigate('/admin/dashboard')
                }
                else if ((authUser && authUser.role === "garbageCollector") || JSON.parse(localStorage.getItem("user")).role === "garbageCollector") {
                    navigate('/collector/profile')
                }
                else if ((authUser && authUser.role === "barangayAdministrator") || JSON.parse(localStorage.getItem("user")).role === "barangayAdministrator") {
                    navigate('/barangay/profile')
                }
                else if ((authUser && authUser.role === "user" )|| JSON.parse(localStorage.getItem("user")).role === "user") {
                    navigate('/newsfeed')
                }
                else if ((authUser && authUser.role === "newUser") || JSON.parse(localStorage.getItem("user")).role === "newUser") {
                    navigate('/newsfeed')
                }
            } else {
                navigate('/register/email-verification')
        ...
        // console.log("localStorageIsAuth", localStorage.getItem("isAuthenticated"))
        // console.log("localStorageUser", JSON.parse(localStorage.getItem("user")).otp_status)

        console.log("isAuthenticated", isAuthenticated)
        console.log("authUser", authUser)
        console.log("authLoading", authLoading)

    }, [dispatch, authError, count, email, authLoading, isAuthenticated, authUser])
    ...
    return (
        <Fragment>
            ...
            <div className="bh-register">
                <h1>Sign up</h1>
                <div className="row register-sections">
                    <div className="col register-section" id="personal-arrow">Personal</div>
                    <div className="col register-section" id="address-arrow">Address</div>
                    <div className="col register-section" id="account-arrow">Account</div>
                    <div className="col register-section" id="verification-arrow">Ts & Cs</div>
                </div>
                <form onSubmit={submitHandler} >
                    {/*Personal*/}
                    <div className="registration-box" id="personal">
                        <div className="row row-cols-lg-4 row-cols-sm-2 row-cols-1 m-auto">
                            <div className="col col-lg-4 col-md-4">
                                <p className="m-0">FIRST NAME</p>
                                <input name="first_name" value={first_name} onChange={onChange} type="text" className="form-control" />
                            </div>
                            <div className="col col-lg-3 col-md-3">
                                <p className="m-0">MIDDLE NAME</p>
                                <input name="middle_name" value={middle_name} onChange={onChange} type="text" className="form-control" />
                            </div>
                            <div className="col col-lg-3 col-md-3">
                                <p className="m-0">LAST NAME</p>
                                <input name="last_name" value={last_name} onChange={onChange} type="text" className="form-control" />
                            </div>
                            <div className="col col-md-2 col-lg-2 col-md-2">
                                <p className="m-0">SUFFIX</p>
                                <input name="suffix" value={suffix} onChange={onChange} type="text" className="form-control" />
                            </div>
                            <div className="col-lg-4 col-md-4">
                                <p className="m-0">BIRTHDATE</p>
                                <input name="birthday" value={birthday} onChange={onChange} type="date" className="datepicker form-control" />
                            </div>
                        </div>
                        <span className="btn bh-submitBtn" onClick={e => nextSection("personal", "address")}>Next</span>
                    </div>
                    {/*Address*/}
                    <div className="registration-box" id="address">
                        <div className="row row-cols-lg-3 row-cols-md-2 row-cols-1 m-auto">
                            <div className="col">
                                <p className="m-0">HOUSE NUMBER</p>
                                <input name="house_number" type="text" value={house_number} onChange={onChange} className="form-control" />
                            </div>
                            <div className="col">
                                <p className="m-0">STREET</p>
                                <input name="street" type="text" value={street} onChange={onChange} className="form-control" />
                            </div>
                            <div className="col">
                                <p className="m-0">BARANGAY</p>
                                <select name="barangay" value={barangay} onChange={onChange} className="form-select">
                        ...
                        <span className="btn bh-submitBtn" onClick={e => nextSection("address", "account")}>Next</span>
                        <span className="btn bh-backBtn" onClick={e => backSection("address", "personal")}>Back</span>
                    </div>
                    {/*Account*/}
                    <div className="registration-box" id="account">
                        <div className="row m-auto">
                            <div className="col-sm-5 col-lg-auto m-auto">
                                <div className="registration-img mb-1" style={{ backgroundImage: `url(${avatarPreview})` }}></div>
                                <input name="avatar" onChange={onChange} style={{ position: "relative", zIndex: "100" }} type="file" className="form-control" accept="image/*" />
                            </div>
                            <div className="col m-auto">
                                <div className="row row-cols-lg-2 row-cols-md-1 row-cols-1 m-auto">
                                    <div className="col">
                                        <p className="m-0">EMAIL
                                            {availableEmail === true ? <span style={{ color: "green", fontWeight: "700", fontSize: "12px" }}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This Email is Available <i className="bi bi-check-circle-fill"></i></span> : ""}
                                            {availableEmail === false ? <span style={{ color: "red", fontWeight: "700", fontSize: "12px" }}>&nbsp;&nbsp;&nbsp;This Email is Already Used <i className="bi bi-x-circle-fill"></i></span> : ""}
                                            {availableEmail === "invalid" ? <span style={{ color: "red", fontWeight: "700", fontSize: "12px" }}>&nbsp;&nbsp;&nbsp;This Email is Invalid Format <i className="bi bi-x-circle-fill"></i></span> : ""}

                                        </p>
                                        <input name="email" value={email} onChange={onChange} type="email" className="form-control" />
                                    </div>
                                    <div className="col">
                                        <p className="m-0">ALIAS/NICKNAME</p>
                                        <input name="alias" value={alias} onChange={onChange} type="text" className="form-control" />
                                    </div>
                                    <div className="col">
                                    ...
                                    <div className="col">
                                        ...
                                        <input name="confirm_password" value={confirm_password} onChange={onChange} type="password" className="form-control" />
                                    </div>
                                    ...
                                    <div className="col" style={{paddingTop:"12px"}}>
                                         ...
                                         <button onClick={() => { imCollector ? setImCollector(false) : setImCollector(true) }} type="button" class="btn btn-success" style={{ height:"fit-content", width:"fit-content", backgroundColor: imCollector ? "#146c43" : "lightGray", border: "unset", color: imCollector ? "white" : "#949494" }}><b>I'am garbage collector</b></button>
                                    {imCollector?
                                        <div>
                                        <p className="m-0">Job Description</p>
                                            <select name="jobDesc" value={jobDesc} onChange={onChange} className="form-select">
                        ...
                        <span className="btn bh-submitBtn" onClick={e => nextSection("account", "verification")}>Next</span>
                        <span className="btn bh-backBtn" onClick={e => backSection("account", "address")}>Back</span>
                    </div>
                    {/*Verification*/}
                    {authLoading ? <Loader /> :
                        <div className="registration-box" id="verification">
                            <div className="row">
                                <div className="col-sm-12">
                                    ...
                                    <div style={{ fontSize: "12px", boxShadow: "0px 6px 6px 0px rgba(0,0,0,.2)", height: "300px", overflowY: "scroll", backgroundColor: "#f0f0f0", padding: "24px" }}>
                                        ...
                                        <p>These Terms of Use constitute a legally binding agreement made between you, whether personally or on behalf of an entity (&ldquo;you&rdquo;) and BasuraHunt (&quot;Company&quot;, &ldquo;we&rdquo;, &ldquo;us&rdquo;, or &ldquo;our&rdquo;), concerning your access to and use of the <a href="https://www.basurahunt.com/">https://www.basurahunt.com</a> website as well as any other media form, media channel, mobile website or mobile application related, linked, or otherwise connected thereto (collectively, the &ldquo;Site&rdquo;). &nbsp;You agree that by accessing the Site, you have read, understood, and agree to be bound by all of these Terms of Use. IF YOU DO NOT AGREE WITH ALL OF THESE TERMS OF USE, THEN YOU ARE EXPRESSLY PROHIBITED FROM USING THE SITE AND YOU MUST DISCONTINUE USE IMMEDIATELY.</p>
                                        ...
                                        <ul>
                                            ...
                                            <li>
                                                <p>Use any information obtained from the Site to harass, abuse, or harm another person.</p>
                                            </li>
                                            <li>
                                    ...
                                    <input type="checkbox" class="form-check-input" value={termAndCond} id="exampleCheck1" onChange={(e) => { setTermAndCond(e.target.checked) }} />
                                    <label class="form-check-label" for="exampleCheck1">I have read and accept the terms of contract</label>
                                </div>
                            </div>
                            <button className="btn bh-submitBtn" disabled={!termAndCond} type="submit">Submit</button>
                            <span className="btn bh-backBtn" onClick={e => backSection("verification", "account")}>Back</span>
                        </div>
                    }
