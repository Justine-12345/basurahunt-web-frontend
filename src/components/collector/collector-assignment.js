...
const CollectorAssignment = () => {
	...
	return(
		<Fragment>
		<MetaData title={`Asssigned`} />
			<div className="bh-container">
				<div>
					<SidebarCollector/>
				</div>
				<div className="bh-container-3 px-3">
					<h3 className="m-0">Assigned Dumps</h3>
					...
					<div className="row row-cols-1 w-100 m-auto pb-3 px-md-5 px-0">
						{
							dumps && dumps.map((dump) => {
								return (
									<Fragment>
										{
											collector(dump) === true && (dump.status !== "Cleaned" && dump.status !== "newReport") && (
												<Link to={`/collector/report/${dump&&dump._id}/${dump&&dump.coordinates&&dump.coordinates.longtitude}/${dump&&dump.coordinates&&dump.coordinates.latitude}/`} className="col mt-3 bh-reports-item">
